...
#define GUI_BUTTON_DOWN         (IsMouseButtonDown(MOUSE_LEFT_BUTTON) || IsGamepadButtonDown(0, GAMEPAD_BUTTON_RIGHT_FACE_DOWN) || (WPAD_ButtonsDown(0) & WPAD_BUTTON_A))
#endif
...
static inline Vector2 WiiGetPointerPosition(void) {
    ir_t ir;
    WPAD_IR(WPAD_CHAN_ALL, &ir);
    if (ir.valid) return (Vector2) { ir.x, ir.y };

    return GetMousePosition();
}
#define GUI_POINTER_POSITION    (WiiGetPointerPosition())
#endif
